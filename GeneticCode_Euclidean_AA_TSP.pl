...
$header=<AA_SCALED>;
	...
	$Hydrophobicity{$aa_data[0]}=$aa_data[2];
...
for(my $aa1=0; $aa1<@amino_acids; $aa1++){
	...
	$d_aa{$amino_acids[$aa1]}{$amino_acids[$aa2]}=sqrt(($Polarity{$amino_acids[$aa1]}-$Polarity{$amino_acids[$aa2]})*($Polarity{$amino_acids[$aa1]}-$Polarity{$amino_acids[$aa2]})+($Hydrophobicity{$amino_acids[$aa1]}-$Hydrophobicity{$amino_acids[$aa2]})*($Hydrophobicity{$amino_acids[$aa1]}-$Hydrophobicity{$amino_acids[$aa2]})+($Volume{$amino_acids[$aa1]}-$Volume{$amino_acids[$aa2]})*($Volume{$amino_acids[$aa1]}-$Volume{$amino_acids[$aa2]})+($Iso_electricity{$amino_acids[$aa1]}-$Iso_electricity{$amino_acids[$aa2]})*($Iso_electricity{$amino_acids[$aa1]}-$Iso_electricity{$amino_acids[$aa2]}));
    }
}
...
foreach my $codon_x (@codons) {
    foreach my $codon_y (@codons) {
...
for(my $aa1=0; $aa1<$N; $aa1++){
    for(my $aa2=0; $aa2<$N; $aa2++){
	$d{$aa1}{$aa2}=$d_aa{$amino_acids[$aa1]}{$amino_acids[$aa2]}/$maximum;
    }
}
open(CODON_PATH, "/Users/oliverattie/code-by-tsp/codon-path-AAA.txt");
while(<CODON_PATH>){
    my @data=split;
    $Codon_Path{$data[2]}{$data[0]}=$data[1];
}
...
#		    for(my $i=0; $i<$N; $i++){
#			print $i." ".$visit[$i]." ".$amino_acids[$visit[$i]]."\n";
#		    }
		$T=$T0/(1.0+$t);
		    print STDERR $T."\n";
		}
...
sub del_energy{
    ...
    for(my $y=0; $y<$N; $y++){
	if($y!=$x){
	    if($i==0){$term2=$term2+($$dref{$x}{$y})*($$Vref{$y}{$i+1}+$$Vref{$y}{$N-1});break;
	    }if($i==$N-1){$term2=$term2+($$dref{$x}{$y})*($$Vref{$y}{0}+$$Vref{$y}{$i-1});break;
...
sub get_codon_dist {
	...
	if($i==0){
	$sum += 5*$transitions{$bases_x[$i]} eq $bases_y[$i] ? 1 : $titv;
	}if($i==1){
