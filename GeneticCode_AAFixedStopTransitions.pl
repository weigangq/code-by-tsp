...
sub get_neighbors{
	...
	my $ti=($transition{$third} eq $rawbase)?1:2;
#	if($ti){
	push(@neighbors, $first.$second.$rawbase);
	push(@ti1, $ti);
#	}
    }
    return (\@neighbors,\@ti1);
}
sub del_energy{
    my $dref=shift;
    my $Vref=shift;
...
#            }else{$term2=$term2+($$dref{$x}{$y})*($$Vref{$y}{$i+1}+$$Vref{$y}{$i-1});break;
	    my ($neighborref,$tiref)=get_neighbors($codons1[$i]);
	    for(my $k=0; $k<@{$neighborref}; $k++){
		$term2=$term2+($$dref{$x}{$y})*($$tiref[$k]*$$Vref{$y}{$CodonNum{$$neighborref[$k]}});
