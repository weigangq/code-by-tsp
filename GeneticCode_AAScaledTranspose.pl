...
open(AA_AS_CITIES, "/Users/oliverattie/aa.as.cities");
while(<AA_AS_CITIES>){
    chomp;
    push(@aa_as_cities, $_);
}
...
for(my $aa1=0; $aa1<@aa_as_cities; $aa1++){
	...
	$d{$aa1}{$aa2}=$d_aa{$aa_as_cities[$aa1]}{$aa_as_cities[$aa2]}/$maximum;
    }
}
...
#	if($dist_combo{$codons[$codon1]}{$codons[$codon2]}>$maximum){
#	$maximum=$dist_combo{$codons[$codon1]}{$codons[$codon2]};
...
#for(my $codon1=0; $codon1<$N; $codon1++){
#    for(my $codon2=0; $codon2<$N; $codon2++){
#	$d{$codon1}{$codon2}=$dist_combo{$codons[$codon1]}{$codons[$codon2]}/$maximum;
#    }
#}
...
#		    for(my $i=0; $i<$N; $i++){
#			print $i." ".$visit[$i]." ".$codons[$visit[$i]]." ".$codon_table{$codons[$visit[$i]]}."\n";
#		    }
		$T=$T0/(1.0+$t);
		    print STDERR $T."\n";
		}
...
for(my $i=0; $i<$N; $i++){
    print $i." ".$codons[$i]." ".$visit[$i]." ".$aa_as_cities[$visit[$i]]."\n";
}
#}
#sub transv_dist{
#    $nuc1=shift;
